...
<body>
 <jsp:include page="/pages/top.jsp"></jsp:include>
    ...
    <div id="main" role="main">
		                              ...
		                              <div class="input-group">
		                                  <div class="input-group-btn">
		                                      ...
		                                      <ul id="issue-condition-selector" class="dropdown-menu">
		                                          <li class="active">
		                                                <a href="javascript:;" data-last-value="allMatchers" ><i class="fa fa-check"></i><s:text name="global.all"/></a>
		                                            </li>
		                                            <li class="divider"></li>
		                                            <li>
                                                    ...
                                                    <!--
		                                            <li>
		                                                <a id="receptor" href="javascript:;" data-last-value="receiver"><s:text name="issue.reciever"/></a>
		                                            </li>
		                                            -->
		                                            <li>
		                                  ...
		                                  <input class="form-control" type="text" placeholder="<s:text name="issue.sys.search.tips"/>" name="allMatchers" data-keyup="true">
		                                  ...
		                                  <div class="input-group-btn">
		                                      <button id="btn-show-more-search" type="button" class="btn btn-default" data-search-mode="combination"><s:text name="global.advance"/></button>
		                                  </div>
		                              </div>
		                            </form>
		                        </div>
		                                ...
		                                <div class="row">
		                                  ...
		                                  <%--           <div class="col-sm-6 col-md-4 col-lg-4">
		                                                <div class="form-group">
															<input id="projectSelector" name="projectNames" type="hidden" data-placeholder="<s:text name="project.name"/>" data-url="../project/displayProjects.jspa" data-options='{"param1": "projectQueryCondition.type", "param2": "projectQueryCondition.name"}' data-param1="system" data-moda="combination">
		                                                </div>
		                                            </div>
		                                            <div class="col-sm-6 col-md-4 col-lg-4">
		                                                <div class="form-group">
															<input id="receiverSelector" name="receivers" type="hidden" data-placeholder="<s:text name="issue.reciever"/>" data-url="<s:url value='/user/findUsersByDevelopRole.jspa'/>" data-moda="combination">
		                                                </div>
		                                            </div> --%>
		                                            <div class="col-sm-6 col-md-4 col-lg-4">
                                                        <div class="form-group">
                                                            <%-- <input name="projectNames" type="hidden" data-placeholder="<s:text name="project.name"/>" data-url="<s:url value='/project/displayProjects.jspa' />" data-options='{"param1": "projectQueryCondition.type", "param2": "projectQueryCondition.name"}' data-param1="card" data-moda="combination"> --%>
                                                    ...
                                                    <div class="col-sm-6 col-md-4 col-lg-4">
                                                        <div class="form-group">
                                                            <input class="form-control" style="padding-left: 5px;" name="productionName" type="text" placeholder="<s:text name="issue.system.name"/>" data-moda="combination">
                                                        </div>
                                                    </div>
		                                    ...
		                                    <div class="col-sm-2 col-md-2 col-lg-2 responsive-alignment">
                                                <div class="pull-right">
    		                                        <button id="btn-show-simple-search" type="button" class="btn btn-default pull-right" data-search-mode="simple"><s:text name="global.base"/></button>
    		...
    		<div class="col-sm-12">
        		<div class="well">
            		<div class="widget-body-toolbar" style="padding:1px 13px 10px 0;">
            		  <div>
            		    <a href="<s:url value="/issue/issues_new_syspro.jspa" />" class="btn btn-success btn-sm pull-right" >
                    ...
                    <div class="dataTables_wrapper">
                		<table id="dt_issues" width="100%;" style="width: 100%; table-layout: fixed; word-wrap:break-word;" class="table table-striped table-hover compact fixedTable dt-responsive no-wrap table-bordered">
                		    <thead>
                		        <tr>
                		            <th><s:text name="issue.code"/></th>
                                    <th class="hidden-xs hidden-sm"><s:text name="project.name"/></th>
                                    <th class="hidden-xs hidden-sm"><s:text name="issue.system.name.abbr"/></th>
		...
		<input id="query-Type" type="hidden" value="system"> <!-- the query type, for system issue, the value should be system, for whilebox issue, it 's whilebox -->
...
//    }
    ...
    function loadDataTableScripts() {
        ...
        function inittable() {
            loadScript("<s:url value='/js/core/issues/issuesSystem.js'/>", init);
        }
    }

    function init()
    {
      issueSystem.initialize();
...
//            var div = $('#moreConditions .table div.nowrap-inline');
    ...
    function initTableEvent()
    {
       /* event for delete button */
        $('#dt_issues tbody > tr').find('td:last> a:last').click(function () {
          issueSystem.deleteIssue({name: 'IDTS'});
        });
        <%-- ----------------------------------------------------------------- --%> 
        //导出excel数据为空时给提醒
          $(".exportToExcelTables").click(function(e){
          	//清空现有弹出框
          	clearSmallBox();
          	window.location = $.url_root+'/issue/export_data_system.jspa';
          });
    ...
    function closedthis() {
        $.smallBox({
            title: "操作成功！已经成功的删除了项目" + $.targetElement + " 以及与其相关联的资料。",
            content: "<i class='fa fa-clock-o'></i> <i>这条消息将在5秒钟后自动消失!</i>",
            color: "#659265",
